...
# defining root directories for datasets as well as other constants
root_1=r'C:\Users\veena\Downloads\missing_data\Patrick_CVAT_dataset'
root_2=r'C:\Users\veena\Downloads\missing_data\Bree_CVAT_dataset'
...
def associate_files(paths):
    ...
    for path in paths:
        ...
        if data.ndim == 1:
            # Handle case where data is 1-dimensional
            # For example, reshape the data if needed
            data = data.reshape(1, -1)  # Reshape to 2D array with one row
...
for sample in dataset:
    ...
    # convert the columns into integer types
        ...
        # Here the variable acc or accuracy
        ...
        sub_df.head()
        sub_df.groupby(['trialid', annotator1])[[annotator2,annotator3]].count()
